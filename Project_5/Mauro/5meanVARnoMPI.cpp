...
/// These fcns calculate the single orbitals, components of the gradients and laplacians
...
ofstream varEfid;
double newpos[10][2];
void MCproc(int Ntherm, int Niter, double delta, double omega, bool verbose, bool save1, bool save2)
{
...
void MCprocRW(int Ntherm, int Niter, double delta, double* omegas, int nWs, bool verbose)
{
    ...
    delete[] sigma;
    delete[] weights;
    gsl_matrix_free(ArwU);
    gsl_matrix_free(ArwD);
    gsl_permutation_free(rwPermU);
    gsl_permutation_free(rwPermD);
}
